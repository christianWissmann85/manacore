...
**Lead Phases:** 2, 3, 5, 6
**Status:** Planned (depends on Track A Phase 1)
...
## Phase 2B: Neural Imitator

**Theme:** "Learn to mimic the master"

**Goal:** Train a simple neural network to predict MCTS moves. This validates the entire ML pipeline before investing in RL.

**Depends on:** Phase 2A (Data Factory) - need 10K games of training data

### Task 2B.1: Model Architecture

**Location:** `packages/ai/src/neural/`
...
- [ ] Create neural network module structure:
  ...
  ├── models/
  │   ├── ImitatorNet.ts     # Simple MLP
  │   └── types.ts           # Tensor types
  ├── training/
  │   ├── DataLoader.ts      # Load NPZ/JSONL
  │   └── Trainer.ts         # Training loop
  └── inference/
      └── NeuralBot.ts       # Bot using trained model
  ```
...
- [ ] Define ImitatorNet architecture:
  ...
  Output: logits over action space (softmax)

  Parameters: ~100K (small, fast)
  ```
...
- [ ] Implement data loader:
  ...
  class TrainingDataLoader {
    ...
    split(ratio: number): [TrainingData, TrainingData];
  }
  ```
...
- [ ] Implement training loop:
  ...
  class Trainer {
    ...
    train(data: TrainingData): TrainingStats;
    evaluate(data: TrainingData): EvalStats;
...
- [ ] Add training script:
  ```bash
  bun scripts/train-imitator.ts \
    --data ./data/mcts-vs-greedy-10k/ \
    --epochs 100 \
    --output ./models/imitator-v1/
  ```
...
- [ ] Implement NeuralBot:
  ...
  class NeuralBot implements Bot {
    ...
    chooseAction(state: GameState, playerId: PlayerId): Action {
      ...
      const masked = maskIllegalActions(logits, legalActions);

      // 4. Sample or argmax
      const actionIdx = argmax(masked);
      return legalActions[actionIdx];
    }
  }
  ```

- [ ] Add to bot registry for experiments
...
- [ ] Win rate vs GreedyBot: >50% (matches training data)
...
- [ ] Compare NeuralBot to MCTS:
  ...
  NeuralBot vs MCTSBot:    Track win rate
  ```

- [ ] Analyze decision quality:
  - How often does NeuralBot agree with MCTS?
