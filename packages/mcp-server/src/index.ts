...
function renderGameState(state: GameState): string {
  ...
  lines.push(`OPPONENT: ❤️ ${opponent.life} | ✋ ${opponent.hand.length} cards | 📚 ${opponent.library.length}`);
  ...
  lines.push(`YOU: ❤️ ${player.life} | ✋ ${player.hand.length} cards | 📚 ${player.library.length}`);
  lines.push(`Lands: ${player.battlefield.filter(c => CardLoader.getById(c.scryfallId)?.type_line.includes('Land')).length}`);
...
class GameSession {
  ...
  constructor(opponentBotName: string, playerDeckName: string, opponentDeckName: string) {
    ...
    const getDeck = (name: string) => {
        switch(name.toLowerCase()) {
            case 'white': return createWhiteDeck();
            case 'blue': return createBlueDeck();
            case 'black': return createBlackDeck();
            case 'red': return createRedDeck();
            case 'green': return createGreenDeck();
            default: return createVanillaDeck();
        }
    };
    ...
    switch (opponentBotName.toLowerCase()) {
      case 'mcts': this.opponentBot = new MCTSBot(); break;
      case 'greedy': this.opponentBot = new GreedyBot(); break;
      default: this.opponentBot = new RandomBot();
    }
  ...
  async playAction(actionIndex: number, reasoning?: string): Promise<string> {
    ...
    if (actionIndex < 0 || actionIndex >= legalActions.length) {
        throw new Error(`Invalid action index ${actionIndex}. Valid range: 0-${legalActions.length - 1}`);
    }
    ...
    // Run Opponent Loop if it's not our priority
    await this.runOpponentLoop();
...
${renderGameState(this.state)}`;
  ...
  async runOpponentLoop() {
      while (!this.state.gameOver && this.state.priorityPlayer === 'opponent') {
          const opponentActions = getLegalActions(this.state, 'opponent');
          if (opponentActions.length === 0) break; // Should not happen ideally

          const botAction = this.opponentBot.chooseAction(this.state, 'opponent');
          // We could record opponent moves too if we set recordPlayer: 'both'
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  try {
      ...
      if (name === 'manacore_start_game') {
        ...
        await activeSession.runOpponentLoop();
