...
class TrainingAnalyzer:
    ...
    def export_for_paper(
        ...
        # Learning curves
        for fmt in formats:
            self.plot_learning_curves(save_path=output_dir / f"learning_curves.{fmt}")
        plt.close('all')
        
        # Comparison matrix
        for fmt in formats:
            self.plot_comparison_matrix(save_path=output_dir / f"comparison_matrix.{fmt}")
        plt.close('all')
        
        # 3D exploration (HTML only)
        self.plot_3d_exploration(save_path=output_dir / "3d_exploration.html")
