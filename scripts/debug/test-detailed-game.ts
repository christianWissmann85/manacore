...
while (!state.gameOver && turnCount < 20) {
  if (state.turnCount > turnCount) {
    ...
    console.log(`  GreedyBot: ${player.life} life, ${playerCreatures.length} creatures, ${player.hand.length} cards`);
    if (playerCreatures.length > 0) {
      console.log(`    Creatures:`);
      for (const c of playerCreatures) {
        const t = CardLoader.getById(c.scryfallId);
        console.log(`      ${t?.name} (${t?.power}/${t?.toughness}) ${c.tapped ? '[tapped]' : '[untapped]'} ${c.summoningSick ? '[summoning sick]' : ''}`);
      }
    }
    console.log(`  RandomBot: ${opponent.life} life, ${opponentCreatures.length} creatures, ${opponent.hand.length} cards`);
  }
  ...
  if (state.phase === 'combat' && state.step === 'declare_blockers' && state.priorityPlayer === 'opponent') {
  ...
  if (state.priorityPlayer === 'player' && state.phase === 'combat') {
    ...
    if (attackOptions.length > 1) {
      ...
      const scoredOptions = attackOptions.slice(0, 5).map(action => {
  ...
  if (actionType === 'PASS_PRIORITY' && prevStep === 'declare_blockers' && newOpponentLife !== prevOpponentLife) {
  ...
  if (newOpponentLife < prevOpponentLife) {
    console.log(`  💥 Opponent took ${prevOpponentLife - newOpponentLife} damage! (${prevOpponentLife} → ${newOpponentLife}) [Priority: ${state.priorityPlayer}]`);
  }
...
console.log(`Final library: ${state.players.player.library.length} vs ${state.players.opponent.library.length}`);
