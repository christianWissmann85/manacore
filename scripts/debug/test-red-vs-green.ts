...
if (drawGames.length > 0) {
  ...
  const avgDrawOpponentLife =
    drawGames.reduce((sum, r) => sum + r.opponentLife, 0) / drawGames.length;
  const avgDrawPlayerLibrary =
    drawGames.reduce((sum, r) => sum + r.playerLibrary, 0) / drawGames.length;
  const avgDrawOpponentLibrary =
    drawGames.reduce((sum, r) => sum + r.opponentLibrary, 0) / drawGames.length;

  console.log('');
  console.log('Draw Game Stats:');
  console.log(`  Average turns:        ${avgDrawTurns.toFixed(1)}`);
  console.log(
    `  Average life (P/O):   ${avgDrawPlayerLife.toFixed(1)} / ${avgDrawOpponentLife.toFixed(1)}`,
  );
  console.log(
    `  Average library (P/O): ${avgDrawPlayerLibrary.toFixed(1)} / ${avgDrawOpponentLibrary.toFixed(1)}`,
  );
}
