...
while (!state.gameOver && turnCount < 200) {
  ...
  const isAttackDecision = legalActions.some(a => a.type === 'DECLARE_ATTACKERS' && a.payload.attackers.length > 0);
  const isBlockDecision = legalActions.some(a => a.type === 'DECLARE_BLOCKERS' && a.payload.blocks.length > 0);
  ...
  if ((isAttackDecision || isBlockDecision) && state.priorityPlayer === 'player') {
    ...
    console.log(`\n${'='.repeat(60)}`);
    console.log(`Turn ${state.turnCount} | ${state.phase} | GreedyBot's ${isAttackDecision ? 'ATTACK' : 'BLOCK'} decision`);
    ...
    if (options.length > 0) {
      ...
      for (let i = 0; i < Math.min(5, scoredOptions.length); i++) {
        ...
        console.log(`${marker} ${i + 1}. ${scoredOptions[i].desc.padEnd(30)} → score: ${scoredOptions[i].score.toFixed(2)}`);
      }
    }
  }
