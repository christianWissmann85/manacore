...
while (!state.gameOver && turnCount < 50) {
  ...
  if (state.turnCount > turnCount) {
    ...
    console.log(
      `  Blue (Greedy): ${player.life} life, ${playerCreatures.length} creatures, ${player.hand.length} cards, ${player.manaPool.blue}U`,
    );

    // Check for Fog Elementals
    const fogElementals = playerCreatures.filter(
      (c) => CardLoader.getById(c.scryfallId)?.name === 'Fog Elemental',
    );
    ...
    console.log(
      `    Lands: ${opponent.battlefield.filter((c) => CardLoader.getById(c.scryfallId)?.type_line?.includes('Land')).length}`,
    );
  }

  const legalActions = getLegalActions(state, state.priorityPlayer);
  if (state.priorityPlayer === 'opponent') {
    // White
    console.log(
      `  [White Legal Actions]: ${legalActions.map((a) => (a.type === 'CAST_SPELL' ? `CAST(${CardLoader.getById(getPlayer(state, 'opponent').hand.find((c) => c.instanceId === a.payload.cardInstanceId)?.scryfallId!)?.name})` : a.type)).join(', ')}`,
    );
  }
  ...
  if (
    ...
    const attackAction = legalActions.find((a) => a.type === 'DECLARE_ATTACKERS') as
      | DeclareAttackersAction
      | undefined;
    if (attackAction && attackAction.payload.attackers.length > 0) {
      // Check if we are attacking with Fog Elemental
      const player = getPlayer(state, 'player');
      const attackingFog = attackAction.payload.attackers.some((id: string) => {
