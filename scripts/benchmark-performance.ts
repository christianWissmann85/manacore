...
async function main() {
  ...
  for (let i = 0; i < bots.length; i++) {
    for (let j = i; j < bots.length; j++) {
      ...
      console.log(`  Speed: ${result.gamesPerSecond.toFixed(2)} games/s (${result.actionsPerSecond.toFixed(2)} actions/s)`);
      console.log(`  Latency: ${result.avgTurnResolutionTimeMs.toFixed(4)} ms/action`);
      console.log(`  Memory: ${result.avgMemoryMb.toFixed(2)} MB avg / ${result.peakMemoryMb.toFixed(2)} MB peak`);
